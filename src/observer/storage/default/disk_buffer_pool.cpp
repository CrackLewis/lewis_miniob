...
Frame *BPFrameManager::begin_purge() {
  ...
  auto purge_finder = [&frame_can_purge](const BPFrameId &frame_id,
                                         Frame *const frame) {
    if (frame->can_purge()) {
...
RC BPFrameManager::free(int file_desc, PageNum page_num, Frame *frame) {
  ...
  if (!found || frame != frame_source) {
    LOG_WARN(
        "failed to find frame or got frame not match. file_desc=%d, "
        "PageNum=%d, frame_source=%p, frame=%p",
...
std::list<Frame *> BPFrameManager::find_list(int file_desc) {
  ...
  auto fetcher = [&frames, file_desc](const BPFrameId &frame_id,
...
////////////////////////////////////////////////////////////////////////////////
DiskBufferPool::DiskBufferPool(BufferPoolManager &bp_manager,
...
RC DiskBufferPool::open_file(const char *file_name) {
  ...
  LOG_INFO("Successfully open %s. file_desc=%d, hdr_frame=%p", file_name,
           file_desc_, hdr_frame_);
  return RC::SUCCESS;
}
...
RC DiskBufferPool::close_file() {
  ...
  // TODO: 理论上是在回放时回滚未提交事务，但目前没有undo
  // log，因此不下刷数据page，只通过redo log回放
  if ((rc = purge_page(0)) != RC::SUCCESS) {
    hdr_frame_->pin_count_++;
    LOG_ERROR("Failed to close %s, due to failed to purge all pages.",
              file_name_.c_str());
    return rc;
  }
...
RC DiskBufferPool::purge_frame(PageNum page_num, Frame *buf) {
  if (buf->pin_count_ > 0) {
    LOG_INFO(
        "Begin to free page %d of %d(file id), but it's pinned, pin_count:%d.",
  ...
  if (buf->dirty_) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN("Failed to flush page %d of %d(file desc) during purge page.",
               buf->page_num(), buf->file_desc_);
      return rc;
    }
  }

  LOG_DEBUG("Successfully purge frame =%p, page %d of %d(file desc)", buf,
            buf->page_num(), buf->file_desc_);
...
RC DiskBufferPool::check_all_pages_unpinned() {
  ...
  for (auto &frame : frames) {
    if (frame->page_num() == BP_HEADER_PAGE && frame->pin_count_ > 1) {
      LOG_WARN(
          "This page has been pinned. file desc=%d, page num:%d, pin count=%d",
...
RC DiskBufferPool::flush_page(Frame &frame) {
  ...
  frame.dirty_ = false;
  LOG_DEBUG("Flush block. file desc=%d, page num=%d", file_desc_,
            page.page_num);
...
RC DiskBufferPool::load_page(PageNum page_num, Frame *frame) {
  ...
  if (ret != 0) {
    LOG_ERROR(
        "Failed to load page %s:%d, due to failed to read data:%s, ret=%d, "
        "page count=%d",
        file_name_.c_str(), page_num, strerror(errno), ret,
        file_header_->allocated_pages);
...
RC BufferPoolManager::open_file(const char *_file_name, DiskBufferPool *&_bp) {
  ...
  fd_buffer_pools_.insert(
      std::pair<int, DiskBufferPool *>(bp->file_desc(), bp));
  _bp = bp;
  return RC::SUCCESS;
}
