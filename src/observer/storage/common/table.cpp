...
RC Table::insert_record(Trx *trx, Record *record) {
  ...
  if (trx != nullptr) {
    ...
    rc = clog_manager_->clog_gen_record(
        CLogType::REDO_INSERT, trx->get_current_id(), clog_record, name(),
        table_meta_.record_size(), record);
    if (rc != RC::SUCCESS) {
...
RC Table::create_index(Trx *trx, const char *index_name,
  ...
  if (table_meta_.index(index_name) != nullptr ||
    ...
    LOG_INFO(
        "Invalid input arguments, table name is %s, index %s exist or "
  ...
  if (!field_meta) {
    LOG_INFO("Invalid input arguments, there is no field of %s in table:%s.",
             attribute_name, name());
    return RC::SCHEMA_FIELD_MISSING;
  }
...
RC Table::delete_record(Trx *trx, Record *record) {
  ...
  if (trx != nullptr) {
    ...
    CLogRecord *clog_record = nullptr;
    rc = clog_manager_->clog_gen_record(CLogType::REDO_DELETE,
...
IndexScanner *Table::find_index_for_scan(const DefaultConditionFilter &filter) {
  ...
  return index->create_scanner(left_key, left_len, left_inclusive, right_key,
