...
bool IndexNodeHandler::validate() const {
  if (parent_page_num() == BP_INVALID_PAGE_NUM) {
    ...
    if (!is_leaf() && size() < 2) {
      LOG_WARN("root page internal node has less than 2 child. size=%d",
               size());
      return false;
    }
  }
...
/////////////////////////////////////////////////////////////////////////////////
LeafIndexNodeHandler::LeafIndexNodeHandler(const IndexFileHeader &header,
                                           Frame *frame)
...
int LeafIndexNodeHandler::max_size() const { return header_.leaf_max_size; }
...
void LeafIndexNodeHandler::insert(int index, const char *key,
                                  const char *value) {
  if (index < size()) {
    memmove(__item_at(index + 1), __item_at(index),
...
void LeafIndexNodeHandler::remove(int index) {
  assert(index >= 0 && index < size());
  if (index < size() - 1) {
    memmove(__item_at(index), __item_at(index + 1),
...
RC LeafIndexNodeHandler::move_half_to(LeafIndexNodeHandler &other,
  ...
  memcpy(other.__item_at(0), this->__item_at(move_index),
         item_size() * (size - move_index));
...
RC LeafIndexNodeHandler::move_to(LeafIndexNodeHandler &other,
                                 DiskBufferPool *bp) {
  memcpy(other.__item_at(other.size()), this->__item_at(0),
         this->size() * item_size());
  ...
  if (next_right_page_num != BP_INVALID_PAGE_NUM) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to fetch next right page. page number:%d. rc=%d:%s",
               next_right_page_num, rc, strrc(rc));
      return rc;
    }
...
std::string to_string(const LeafIndexNodeHandler &handler,
                      const KeyPrinter &printer) {
  std::stringstream ss;
...
bool LeafIndexNodeHandler::validate(const KeyComparator &comparator,
                                    DiskBufferPool *bp) const {
  ...
  if (index_in_parent < 0) {
    LOG_WARN(
        "invalid leaf node. cannot find index in parent. this page num=%d, "
        "parent page num=%d",
  ...
  if (0 != index_in_parent) {
    ...
    if (cmp_result < 0) {
      LOG_WARN(
          "invalid leaf node. first item should be greate than or equal to "
          "parent item. "
  ...
  if (index_in_parent < parent_node.size() - 1) {
    int cmp_result = comparator(__key_at(size() - 1),
                                parent_node.key_at(index_in_parent + 1));
    if (cmp_result >= 0) {
      LOG_WARN(
          "invalid leaf node. last item should be less than the item at the "
          "first after item in parent."
...
std::string to_string(const InternalIndexNodeHandler &node,
                      const KeyPrinter &printer) {
  std::stringstream ss;
...
void InternalIndexNodeHandler::create_new_root(PageNum first_page_num,
                                               const char *key,
                                               PageNum page_num) {
 ...
 */
void InternalIndexNodeHandler::insert(const char *key, PageNum page_num,
                                      const KeyComparator &comparator) {
  ...
  if (insert_position < size()) {
    memmove(__item_at(insert_position + 1), __item_at(insert_position),
            (size() - insert_position) * item_size());
  }
...
int InternalIndexNodeHandler::lookup(
    const KeyComparator &comparator, const char *key,
 ...
 */
RC InternalIndexNodeHandler::copy_from(const char *items, int num,
                                       DiskBufferPool *disk_buffer_pool) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN(
          "failed to set child's page num. child page num:%d, this page "
          "num=%d, rc=%d:%s",
...
bool InternalIndexNodeHandler::validate(const KeyComparator &comparator,
                                        DiskBufferPool *bp) const {
  bool result = IndexNodeHandler::validate();
  if (false == result) {
    return false;
  ...
  for (int i = 0; result && i < node_size; i++) {
    ...
    if (page_num < 0) {
      LOG_WARN("this page num=%d, got invalid child page. page num=%d",
      ...
      if (rc != RC::SUCCESS) {
        LOG_WARN(
            "failed to fetch child page while validate internal page. page "
            "num=%d, rc=%d:%s",
        ...
        if (child_node.parent_page_num() != this->page_num()) {
          LOG_WARN(
              "child's parent page num is invalid. child page num=%d, parent "
              "page num=%d, this page num=%d",
              child_node.page_num(), child_node.parent_page_num(),
              this->page_num());
          result = false;
        }
  ...
  if (index_in_parent < 0) {
    LOG_WARN(
        "invalid internal node. cannot find index in parent. this page num=%d, "
        "parent page num=%d",
        this->page_num(), parent_page_num);
    bp->unpin_page(parent_frame);
    return false;
  ...
  if (0 != index_in_parent) {
    ...
    if (cmp_result < 0) {
      LOG_WARN(
          "invalid internal node. the second item should be greate than or "
          "equal to parent item. "
          "this page num=%d, parent page num=%d, index in parent=%d",
          this->page_num(), parent_node.page_num(), index_in_parent);
      bp->unpin_page(parent_frame);
      return false;
  ...
  if (index_in_parent < parent_node.size() - 1) {
    ...
    if (cmp_result >= 0) {
      LOG_WARN(
          "invalid internal node. last item should be less than the item at "
          "the first after item in parent."
          "this page num=%d, parent page num=%d, parent item to compare=%d",
          this->page_num(), parent_node.page_num(), index_in_parent + 1);
      bp->unpin_page(parent_frame);
      return false;
...
RC BplusTreeHandler::create(const char *file_name, AttrType attr_type,
                            int attr_length, int internal_max_size /* = -1*/,
                            int leaf_max_size /* = -1 */) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to allocate header page for bplus tree. rc=%d:%s", rc,
             strrc(rc));
...
RC BplusTreeHandler::open(const char *file_name) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("Failed to get first page file name=%s, rc=%d:%s", file_name, rc,
             strrc(rc));
    bpm.close_file(file_name);
    return rc;
  }
...
RC BplusTreeHandler::print_internal_node_recursive(Frame *frame) {
  ...
  for (int i = 0; i < node_size; i++) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to fetch child page. page id=%d, rc=%d:%s", page_num, rc,
               strrc(rc));
      disk_buffer_pool_->unpin_page(frame);
      return rc;
    }
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to print node. page id=%d, rc=%d:%s",
               child_frame->page_num(), rc, strrc(rc));
      disk_buffer_pool_->unpin_page(frame);
      return rc;
    }
  }
...
RC BplusTreeHandler::print_tree() {
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to fetch page. page id=%d, rc=%d:%s", page_num, rc,
             strrc(rc));
    return rc;
  }
...
RC BplusTreeHandler::print_leafs() {
  ...
  while (frame->page_num() != BP_INVALID_PAGE_NUM) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to get next page. page id=%d, rc=%d:%s", next_page_num,
               rc, strrc(rc));
      return rc;
    }
  }
...
bool BplusTreeHandler::validate_node_recursive(Frame *frame) {
  ...
  if (node.is_leaf()) {
    ...
    for (int i = 0; result && i < internal_node.size(); i++) {
      ...
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to fetch child page.page id=%d, rc=%d:%s", page_num,
                 rc, strrc(rc));
        result = false;
        break;
      }
...
bool BplusTreeHandler::validate_leaf_link() {
  ...
  if (leaf_node.prev_page() != prev_page_num) {
    LOG_WARN(
        "invalid page. current_page_num=%d, prev page num should be %d but got "
        "%d",
  ...
  memcpy(prev_key, leaf_node.key_at(leaf_node.size() - 1),
  ...
  while (result && next_page_num != BP_INVALID_PAGE_NUM) {
    ...
    if (rc != RC::SUCCESS) {
      free_key(prev_key);
      LOG_WARN("failed to fetch next page. page num=%d, rc=%d:%s",
               next_page_num, rc, strrc(rc));
      return false;
    }
    ...
    if (leaf_node.prev_page() != prev_page_num) {
      LOG_WARN(
          "invalid page. current_page_num=%d, prev page num should be %d but "
          "got %d",
          frame->page_num(), prev_page_num, leaf_node.prev_page());
      result = false;
    }
...
bool BplusTreeHandler::validate_tree() {
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to fetch root page. page id=%d, rc=%d:%s",
             file_header_.root_page, rc, strrc(rc));
    return rc;
  }
...
RC BplusTreeHandler::find_leaf(const char *key, Frame *&frame) {
  return find_leaf_internal(
      [&](InternalIndexNodeHandler &internal_node) {
        return internal_node.value_at(
            internal_node.lookup(key_comparator_, key));
      },
      frame);
}
...
RC BplusTreeHandler::insert_entry_into_leaf_node(Frame *frame, const char *key,
                                                 const RID *rid) {
  LeafIndexNodeHandler leaf_node(file_header_, frame);
  bool exists = false;
  ...
  if (insert_position < leaf_node.size()) {
    ...
    new_index_node.insert(insert_position - leaf_node.size(), key,
                          (const char *)rid);
  }
...
RC BplusTreeHandler::insert_entry_into_parent(Frame *frame, Frame *new_frame,
                                              const char *key) {
  RC rc = RC::SUCCESS;
  ...
  if (parent_page_num == BP_INVALID_PAGE_NUM) {
    ...
    if (node.size() < node.max_size()) {
      ...
      // we should split the node and insert the entry and then insert new entry
      ...
      if (rc != RC::SUCCESS) {
        ...
        rc = insert_entry_into_parent(parent_frame, new_parent_frame,
                                      new_node.key_at(0));
      }
    }
  }
...
RC BplusTreeHandler::split(Frame *frame, Frame *&new_frame) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN(
        "Failed to split index page due to failed to allocate page, rc=%d:%s",
        rc, strrc(rc));
    return rc;
  }
...
RC BplusTreeHandler::create_new_tree(const char *key, const RID *rid) {
  ...
  if (file_header_.root_page != BP_INVALID_PAGE_NUM) {
    rc = RC::INTERNAL;
    LOG_WARN("cannot create new tree while root page is valid. root page id=%d",
...
void BplusTreeHandler::free_key(char *key) { mem_pool_item_->free(key); }
...
RC BplusTreeHandler::insert_entry(const char *user_key, const RID *rid) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("Failed to find leaf %s. rc=%d:%s", rid->to_string().c_str(), rc,
             strrc(rc));
    mem_pool_item_->free(key);
    return rc;
  }
  ...
  if (rc != RC::SUCCESS) {
    LOG_TRACE("Failed to insert into leaf of index, rid:%s",
              rid->to_string().c_str());
    disk_buffer_pool_->unpin_page(frame);
    mem_pool_item_->free(key);
    // disk_buffer_pool_->check_all_pages_unpinned(file_id_);
...
RC BplusTreeHandler::get_entry(const char *user_key, int key_len,
                               std::list<RID> &rids) {
  BplusTreeScanner scanner(*this);
  RC rc = scanner.open(user_key, key_len, true /*left_inclusive*/, user_key,
                       key_len, true /*right_inclusive*/);
  if (rc != RC::SUCCESS) {
...
RC BplusTreeHandler::adjust_root(Frame *root_frame) {
  ...
  if (root_node.is_leaf()) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN("failed to fetch child page. page num=%d, rc=%d:%s",
               child_page_num, rc, strrc(rc));
      return rc;
    }
...
RC BplusTreeHandler::coalesce_or_redistribute(Frame *frame) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to fetch parent page. page id=%d, rc=%d:%s",
             parent_page_num, rc, strrc(rc));
    disk_buffer_pool_->unpin_page(frame);
    return rc;
  }
  ...
  if (parent_index_node.value_at(index) != frame->page_num()) {
    LOG_ERROR(
        "lookup return an invalid value. index=%d, this page num=%d, but got "
        "%d",
  ...
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to fetch neighbor page. page id=%d, rc=%d:%s",
             neighbor_page_num, rc, strrc(rc));
  ...
  if (index_node.size() + neighbor_node.size() > index_node.max_size()) {
    rc = redistribute<IndexNodeHandlerType>(neighbor_frame, frame, parent_frame,
                                            index);
  } else {
    rc = coalesce<IndexNodeHandlerType>(neighbor_frame, frame, parent_frame,
                                        index);
  }
  return rc;
}
...
template <typename IndexNodeHandlerType>
RC BplusTreeHandler::coalesce(Frame *neighbor_frame, Frame *frame,
                              Frame *parent_frame, int index) {
  IndexNodeHandlerType neighbor_node(file_header_, neighbor_frame);
  ...
  if (left_node.is_leaf()) {
    ...
    if (next_right_page_num != BP_INVALID_PAGE_NUM) {
      Frame *next_right_frame;
      rc = disk_buffer_pool_->get_this_page(next_right_page_num,
                                            &next_right_frame);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to fetch next right page. page number:%d. rc=%d:%s",
                 next_right_page_num, rc, strrc(rc));
...
template <typename IndexNodeHandlerType>
RC BplusTreeHandler::redistribute(Frame *neighbor_frame, Frame *frame,
                                  Frame *parent_frame, int index) {
  InternalIndexNodeHandler parent_node(file_header_, parent_frame);
  IndexNodeHandlerType neighbor_node(file_header_, neighbor_frame);
  IndexNodeHandlerType node(file_header_, frame);
...
RC BplusTreeScanner::open(const char *left_user_key, int left_len,
                          bool left_inclusive, const char *right_user_key,
  ...
  if (nullptr == left_user_key) {
    ...
    if (left_index >=
      ...
      rc = tree_handler_.disk_buffer_pool_->get_this_page(next_page_num,
                                                          &left_frame_);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to fetch next page. page num=%d, rc=%d:%s",
                 next_page_num, rc, strrc(rc));
  ...
  if (nullptr == right_user_key) {
    ...
    if (tree_handler_.file_header_.attr_type == CHARS) {
      rc = fix_user_key(right_user_key, right_len, false /*want_greater*/,
                        &fixed_right_key, &should_include_after_fix);
      if (rc != RC::SUCCESS) {
    ...
    int right_index =
        right_node.lookup(tree_handler_.key_comparator_, right_key);
    tree_handler_.free_key(right_key);
    ...
    if (right_index > 0) {
      ...
      rc = tree_handler_.disk_buffer_pool_->get_this_page(prev_page_num,
                                                          &right_frame_);
      if (rc != RC::SUCCESS) {
        LOG_WARN("failed to fetch prev page num. page num=%d, rc=%d:%s",
                 prev_page_num, rc, strrc(rc));
        return rc;
      }
...
RC BplusTreeScanner::next_entry(RID *rid) {
  ...
  if (left_frame_->page_num() != right_frame_->page_num()) {
    ...
    if (page_num == BP_INVALID_PAGE_NUM) {
      ...
      rc = tree_handler_.disk_buffer_pool_->get_this_page(page_num,
                                                          &left_frame_);
      if (rc != RC::SUCCESS) {
        left_frame_ = nullptr;
        LOG_WARN("failed to fetch next page. page num=%d, rc=%d:%s", page_num,
                 rc, strrc(rc));
        return rc;
      }
...
RC BplusTreeScanner::fix_user_key(const char *user_key, int key_len,
                                  bool want_greater, char **fixed_key,
