 ...
 public:
  ...
  int operator()(const char *v1, const char *v2) const {
    switch (attr_type_) {
      ...
      case CHARS: {
        return compare_string((void *)v1, attr_length_, (void *)v2,
                              attr_length_);
      }
      default: {
...
class LeafIndexNodeHandler : public IndexNodeHandler {
 public:
  ...
  friend std::string to_string(const LeafIndexNodeHandler &handler,
...
class InternalIndexNodeHandler : public IndexNodeHandler {
 public:
  ...
  void init_empty();
  void create_new_root(PageNum first_page_num, const char *key,
                       PageNum page_num);

  void insert(const char *key, PageNum page_num,
  ...
  friend std::string to_string(const InternalIndexNodeHandler &handler,
                               const KeyPrinter &printer);
...
class BplusTreeHandler {
 public:
   ...
   * return true means current tree is valid, return false means current tree is
   * invalid.
   * @return
   */
  bool validate_tree();
 ...
 protected:
  ...
  RC find_leaf_internal(const std::function<PageNum(InternalIndexNodeHandler &)>
                            &child_page_getter,
                        Frame *&frame);

  RC insert_into_parent(PageNum parent_page, Frame *left_frame,
                        const char *pkey, Frame &right_frame);

  RC delete_entry_internal(Frame *leaf_frame, const char *key);

  RC insert_into_new_root(Frame *left_frame, const char *pkey,
                          Frame &right_frame);
  ...
  template <typename IndexNodeHandlerType>
  RC coalesce(Frame *neighbor_frame, Frame *frame, Frame *parent_frame,
              int index);
  template <typename IndexNodeHandlerType>
  RC redistribute(Frame *neighbor_frame, Frame *frame, Frame *parent_frame,
                  int index);

  RC insert_entry_into_parent(Frame *frame, Frame *new_frame, const char *key);
  RC insert_entry_into_leaf_node(Frame *frame, const char *pkey,
                                 const RID *rid);
